...
/** Applies the relevant business rules for an application or amendment, with inputs being whatever existing protections
  * are in place
  */
trait ApplicationRules {

  /** @param existingProtections
    *   all existing protections for the individual at the time of the amendment or application
    * @return
    *   the outcome of the business rules check in the form of a notification ID - should be greater than 1 but 0 is
    *   reserved for the case where something seriously goes wrong (most likely invalid existing protection data)
    */
...
trait AmendmentRules {
  ...
  /** @param relevantAmount
    ...
    *   the current status of the protection to be amended
...
object IP2014AmendmentRules extends AmendmentRules {
  ...
  override def check(relevantAmount: Double, otherExistingProtections: List[Protection]) = {
    ...
    otherOpenProtectionOpt
      .map { openProtection: Protection =>
